...
{
    int ret = mergesort(empty_array, 0, sizeof(int), int_cmp);
...
char *test_one()
{
    int ret = mergesort(one_array, 1, sizeof(int), int_cmp);
...
char *test_two()
{
    size_t size = 2;
#ifdef EXTRA_DEBUG
    printf("before sort ints two -- ");
    print_ints(int_test_in_order, size, 0);
#endif
    int ret = mergesort(two_array, size, sizeof(int), int_cmp);
#ifdef EXTRA_DEBUG
    printf("after sort ints two -- ");
    print_ints(int_test_in_order, size, 0);
#endif
...
char *test_ints_in_order()
{
    ...
    print_ints(int_test_in_order, size, 0);
#endif
    int ret_sort = mergesort(int_test_in_order, size, sizeof(int), int_cmp);
    //int ret_sort = __mergesort(int_test_in_order, size, sizeof(int), int_cmp, print_ints);
#ifdef EXTRA_DEBUG
...
#endif
    int ret_sort = mergesort(int_test_reversed, size, sizeof(int), int_cmp);
    //int ret_sort = __mergesort(int_test_reversed, size, sizeof(int), int_cmp, print_ints);
#ifdef EXTRA_DEBUG
...
#endif
    int ret_struct_sort = mergesort(gen_array, size, sizeof(test_struct), struct_cmp);
#ifdef EXTRA_DEBUG
